  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      try {
        ...
        if (typeof window !== 'undefined') {
          const errorMessage = error.message.includes('подключиться к серверу') 
            ? 'Не удается подключиться к серверу. Обратитесь к администратору.'
            : `Ошибка загрузки данных: ${error.message}`;
          
          // Можно добавить уведомление в UI вместо alert
          console.error('🚨 Пользователю:', errorMessage);
        }
      }
    };

    // Загружаем данные только после монтирования
    if (isMounted) {
      loadData();
    }
  }, [isMounted]);
