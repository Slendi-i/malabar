...
// 🚨 ЭКСТРЕННОЕ РЕШЕНИЕ: Максимально простой endpoint для координат
app.patch('/api/coordinates/:id', (req, res) => {
  console.log('🚨 EMERGENCY ENDPOINT START');
  
  try {
    const playerId = parseInt(req.params.id);
    const { x, y } = req.body;
    
    console.log(`🚨 EMERGENCY: Данные - playerId: ${playerId}, x: ${x}, y: ${y}`);
    console.log(`🚨 EMERGENCY: Типы - playerId: ${typeof playerId}, x: ${typeof x}, y: ${typeof y}`);
    
    // Минимальная валидация
    if (!playerId || playerId <= 0) {
      console.log('🚨 EMERGENCY ERROR: Invalid player ID');
      return res.status(400).json({ error: 'Invalid player ID' });
    }
    
    if (x === undefined || y === undefined) {
      console.log('🚨 EMERGENCY ERROR: Missing coordinates');
    ...
    if (isNaN(validX) || isNaN(validY)) {
      console.log('🚨 EMERGENCY ERROR: Invalid coordinate values');
      return res.status(400).json({ error: 'Invalid coordinate values' });
    }
    
    console.log(`🚨 EMERGENCY: Выполняем UPDATE для игрока ${playerId} с координатами (${validX}, ${validY})`);
    
    // МАКСИМАЛЬНО ПРОСТОЙ SQL - только координаты
    const sql = 'UPDATE players SET x = ?, y = ? WHERE id = ?';
    const params = [validX, validY, playerId];
    
    console.log('🚨 EMERGENCY: SQL:', sql);
    console.log('🚨 EMERGENCY: Params:', params);
    
    db.run(sql, params, function(err) {
      if (err) {
        console.error(`🚨 EMERGENCY DB ERROR:`, err);
        console.error(`🚨 EMERGENCY DB ERROR Details:`, {
          message: err.message,
          code: err.code,
          errno: err.errno
        });
      ...
      console.log(`🚨 EMERGENCY: Changes: ${this.changes}`);
      
      if (this.changes === 0) {
        console.log('🚨 EMERGENCY: No rows updated - player not found');
        return res.status(404).json({ error: 'Player not found' });
      }
      
      console.log(`🚨 EMERGENCY SUCCESS: Координаты обновлены для игрока ${playerId}`);
      
      // Простая трансляция
      try {
        const coordinatesData = { id: playerId, x: validX, y: validY };
        broadcastUpdate('coordinates', coordinatesData);
        console.log('🚨 EMERGENCY: WebSocket broadcast sent');
      } catch (broadcastErr) {
        console.error('🚨 EMERGENCY: Broadcast error:', broadcastErr);
        // Не падаем если broadcast не работает
      }
      ...
      console.log('🚨 EMERGENCY: Sending response:', response);
      res.json(response);
    });
    
  } catch (error) {
    console.error('🚨 EMERGENCY CATCH ERROR:', error);
    console.error('🚨 EMERGENCY CATCH Stack:', error.stack);
    res.status(500).json({ 
      error: 'Server error in emergency endpoint',
